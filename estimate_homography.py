...
def fit_image_in_target_space(img_src, img_dst, mask, H):
    ...
    get_pixel_val(img_dst, img_src, pts, out_src)
...
def get_pixel_val(img_dst, img_src, pts, out_src):
    """

    :param img_dst:
    :param pts: pts for img_dst
    ...
    weight[:, 0] = 1/np.linalg.norm(tl-out_src[:, ::-1], axis=1)
    weight[:, 1] = 1/np.linalg.norm(tr-out_src[:, ::-1], axis=1)
    weight[:, 2] = 1/np.linalg.norm(bl-out_src[:, ::-1], axis=1)
    weight[:, 3] = 1/np.linalg.norm(br - out_src[:, ::-1], axis=1)
